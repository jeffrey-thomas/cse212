...
public class LinkedList : IEnumerable<int>
{
    ...
    {
        //Find the first node that contains 'value'
        Node? curr = _head;
        while(curr is not null && curr.Data!=value)
            curr = curr.Next;

        if(curr is not null){           //Check if a node was found that conatins 'value'
            
            if(curr == _head)           //Check if the node that was found is the head
                RemoveHead();
            else if( curr == _tail)     //Check if the node that was found is the tail
                RemoveTail();
            else{                       //Otherwise remove from the middle
    ...
    public void Replace(int oldValue, int newValue)
    {
        ...
        Node? curr = _head;
        while(curr is not null){
            if(curr.Data == oldValue)   //Check if data matches 'oldValue'
                curr.Data = newValue;       //if so, update to 'newValue' 
            curr = curr.Next;
        }
    }
